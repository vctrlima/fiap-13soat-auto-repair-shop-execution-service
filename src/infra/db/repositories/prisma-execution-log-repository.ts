...
export class PrismaExecutionLogRepository
  implements
    ...
    CompleteWorkOrderExecutionRepository
{
  async create(params: CreateExecutionLogsRepository.Params): Promise<CreateExecutionLogsRepository.Result> {
  ...
  async getByWorkOrderId(params: GetExecutionLogsByWorkOrderIdRepository.Params): Promise<GetExecutionLogsByWorkOrderIdRepository.Result> {
    const logs = await prisma.executionLog.findMany({
      where: { workOrderId: params.workOrderId },
      orderBy: { createdAt: 'asc' },
    });
  ...
  async updateStatus(params: UpdateExecutionLogStatusRepository.Params): Promise<UpdateExecutionLogStatusRepository.Result> {
    const now = new Date();
    const updateData: any = { status: params.status };

    if (params.status === 'IN_PROGRESS') updateData.startedAt = now;
    if (params.status === 'COMPLETED' || params.status === 'FAILED') updateData.completedAt = now;
  ...
  async complete(params: CompleteWorkOrderExecutionRepository.Params): Promise<CompleteWorkOrderExecutionRepository.Result> {
    const logs = await prisma.executionLog.findMany({
      where: { workOrderId: params.workOrderId },
    });

    const allCompleted = logs.every((l) => l.status === 'COMPLETED');
    const completedServices = logs.filter((l) => l.status === 'COMPLETED').map((l) => l.serviceName);
  ...
  private async updateServiceMetrics(serviceId: string, serviceName: string): Promise<void> {
    ...
    await prisma.serviceMetrics.upsert({
      where: { serviceId },
      update: { totalExecutions, averageExecutionTimeInMinutes: avgTime, successRate },
      create: { serviceId, serviceName, totalExecutions, averageExecutionTimeInMinutes: avgTime, successRate },
    });
  }
}
